...
s_code_image=$oemirot_appli_secure_primary
ns_code_image=$oemirot_appli_non_secure_primary
one_code_image=$oemirot_appli_assembly_sign_primary
l_code_image=$oemirot_loader
...
if  [ $primary_only -eq 1 ]; then
    ...
    echo "$action"
    "$stm32programmercli" $connect_no_reset -d "$appli_dir/Binary/$l_code_image" $loaderaddress -v --skipErase
    if [ $? -ne 0 ]; then error; return 1; fi
...
echo "$action"
"$stm32programmercli" "$connect_no_reset" -d "$cube_fw_path/${oemirot_boot_path_project}/Binary/OEMiROT_Boot.bin" $bootaddress -v
if [ $? -ne 0 ]; then error; return 1; fi
