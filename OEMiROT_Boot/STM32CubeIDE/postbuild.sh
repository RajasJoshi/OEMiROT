...
s_ld_file="$appli_dir/Secure/STM32H563ZITX_FLASH.ld"
ns_ld_file="$appli_dir/NonSecure/STM32H563ZITX_FLASH.ld"
...
#
$python$applicfg flash --layout $preprocess_bl2_file -b loaderaddress -m RE_IMAGE_FLASH_LOADER_ADDRESS $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi
...
#
$python$applicfg setob --layout $preprocess_bl2_file -b wrpgrp2 -ms RE_LOADER_WRP_START -me RE_LOADER_WRP_END -msec RE_FLASH_PAGE_NBR -d 0x8000 $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi
...
#
$python$applicfg flash --layout $preprocess_bl2_file -b slot0 -m RE_IMAGE_FLASH_ADDRESS_SECURE $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg flash --layout $preprocess_bl2_file -b slot1 -m RE_IMAGE_FLASH_ADDRESS_NON_SECURE $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg flash --layout $preprocess_bl2_file -b slot4 -m RE_IMAGE_FLASH_ADDRESS_DATA_SECURE $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg flash --layout $preprocess_bl2_file -b slot5 -m RE_IMAGE_FLASH_ADDRESS_DATA_NON_SECURE $update --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi
...
#
$python$applicfg xmlparam --layout  $preprocess_bl2_file -m RE_PRIMARY_ONLY -n "Slot Option" -t Data -c --primary-only -h 1 -d "" $s_code_xml --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg xmlparam --layout  $preprocess_bl2_file -m RE_PRIMARY_ONLY -n "Slot Option" -t Data -c --primary-only -h 1 -d "" $ns_code_xml --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg xmlparam --layout  $preprocess_bl2_file -m RE_PRIMARY_ONLY -n "Slot Option" -t Data -c --primary-only -h 1 -d "" $s_data_xml --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi

#
$python$applicfg xmlparam --layout  $preprocess_bl2_file -m RE_PRIMARY_ONLY -n "Slot Option" -t Data -c --primary-only -h 1 -d "" $ns_data_xml --vb >> $current_log_file 2>&1
if [ $? != 0 ]; then error; fi
...
if [ -f $appli_flash_layout ]; then
  ...
  #
  $python$applicfg setdefine --layout $preprocess_bl2_file -m RE_PRIMARY_ONLY -n MCUBOOT_PRIMARY_ONLY -v 1 $appli_flash_layout --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi
...
#=================================================================================================
# Bypass configuration of loader if not present
if [ -f $loader_dir ]; then

  loader_flash_layout="$loader_dir\Inc\appli_flash_layout.h"
  l_main="$loader_dir\Inc\main.h"
  l_linker_file="$loader_dir\STM32CubeIDE\STM32H563ZITX_FLASH.ld"
  ...
  #
  $python$applicfg  definevalue --layout $preprocess_bl2_file -m RE_S_DATA_IMAGE_NUMBER -n MCUBOOT_S_DATA_IMAGE_NUMBER $loader_flash_layout >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi
  ...
  #
  $python$applicfg  definevalue --layout $preprocess_bl2_file -m RE_IMAGE_NON_SECURE_IMAGE_SIZE -n FLASH_NS_PARTITION_SIZE $loader_flash_layout --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi
  ...
  #
  $python$applicfg  definevalue --layout $preprocess_bl2_file -m RE_IMAGE_FLASH_LOADER_OFFSET -n S_CODE_OFFSET $l_main --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi

  #
  $python$applicfg  definevalue --layout $preprocess_bl2_file -m RE_LOADER_S_CODE_SIZE -n S_CODE_SIZE $l_main --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi

  #
  $python$applicfg  linker --layout $preprocess_bl2_file -m RE_IMAGE_FLASH_LOADER_OFFSET -n S_CODE_OFFSET $l_linker_file --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi

  #
  $python$applicfg  linker --layout $preprocess_bl2_file -m  RE_LOADER_S_CODE_SIZE -n S_CODE_SIZE $l_linker_file --vb >> $current_log_file 2>&1
  if [ $? != 0 ]; then error; fi
